...
5.  **計算餘弦距離**：
    ...
    - 計算所有**類間**物件配對的餘弦距離。
6.  **產出結果**：
    - 將計算出的距離分別儲存為 `intra_class_cosine_distances.csv` 和 `inter_class_cosine_distances.csv`。
    - 繪製一張結合了類內與類間距離分佈的圖表 `cosine_distance_distribution.png`。
...
```bash
yolo12_env\Scripts\python.exe calculate_cosine_similarity.py
```
